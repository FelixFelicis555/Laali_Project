...
router.post("/admin/mentee-mentor", checkAuth, async(req, res) => {
    ...
    try {
        let suc1 = await user.updateOne({ email: mentorEmail }, { $set: { menteeEmail: menteeEmail, mentee: true } });
    } catch (e1) {
        console.log("error ", e1);
    }
    try {
        let suc2 = await user.updateOne({ email: menteeEmail }, { $set: { mentorEmail: mentorEmail, mentor: true } });
    } catch (e2) {
        console.log("error ", e2);
    }
    res.redirect("/admin/dashboard");
...
router.post("/admin/verifyAlumni", checkAuth, async(req, res) => {
    ...
    const suc = await alumni.updateOne({ phone: phone }, { $set: { isPending: false } });
    phoneno = +phone;
