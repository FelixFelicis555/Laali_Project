...
function checkAuth(req, res, next) {
    if (req.isAuthenticated()) {
        res.set('Cache-Control', 'no-cache, private, no-store, must-revalidate, post-check=0, pre-check=0'); // disable-caching
        next();
    } else {
...
router.get('/user/send-verification-email', checkAuth, async(req, res) => {
    if (req.user.isVerified || req.user.provider == 'google') {
        ...
        await resetToken({ token: token, emai: req.user.emai }).save();
...
router.get('/user/reset-password', async(req, res) => {
    const token = req.query.token;
    if (token) {
        var checkl = await resetToken.findOne({ token: token });
        if (check) {
...
router.post('/user/reset-password', async(req, res) => {
    ...
    if (!password || !password2 || (password2 != password)) {
        res.render('forgot-password.ejs', { reset: true, err: "Password Don't Match !", email: email });
    } else {
